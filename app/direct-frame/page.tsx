...
export default function DirectFramePage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const frameUrl = searchParams.get('url');
  ...
  // Validate the URL and set display URL
  useEffect(() => {
    if (!frameUrl) {
      setError("No frame URL provided");
      setIsLoading(false);
      return;
    }
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col justify-center items-center h-screen bg-[#10001D]">
        ...
        <p className="text-white text-lg">Loading frame from URL...</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center py-10 bg-[#10001D] min-h-screen text-white">
        <div className="bg-red-500 text-white px-4 py-3 rounded mb-4">
          <p>Error: {error || 'Frame URL not provided'}</p>
        </div>
        <button 
          onClick={() => router.push('/')} 
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center">
              <div className="w-12 h-12 rounded-full mr-4 bg-[#2A2A3C] flex items-center justify-center text-lg">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
                </svg>
              </div>
              <div>
                <h1 className="text-2xl font-bold">Direct Frame</h1>
                <p className="text-[#8C56FF] text-sm">
                  @Direct Link
                </p>
              </div>
            </div>
